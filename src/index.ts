...
import { promises as fsPromises, statSync, Stats } from 'fs';
 ...
 */
export async function* findAsync(input: string, base?: string, fileName?: string): AsyncGenerator<string, void, void> {
 ...
 * @param fileName The filename to search for (default: `package.json`)
 */
export function* findSync(input: string, base?: string, fileName?: string): Generator<string, void, void> {
  log(`findSync: called with "${input}" and "${base}" and "${fileName}"`);
 ...
 */
function statPathSync(path: string): Stats | undefined {
  try {
 ...
 * @throws if the error is not "ENOENT"
 */
async function statPathAsync(path: string): Promise<Stats | undefined> {
